...
class WeixinAddonModel extends WeixinModel {
	...
	function scan($dataArr, $keywordArr = array(), $config = array()) {
		...
		//$this->replyText($qr ['addon']);

		if ($qr ['addon'] == 'Orienteering') { // 定向赛
			$task_id = $qr['aim_id'];
			$openid = $map ['openid'] = $dataArr ['FromUserName'];
			$token =  $map ['token'] = get_token ();
			$followId = M ( 'public_follow' )->where ( $map )->getField ( 'uid' );
			$create_time = $dataArr['CreateTime'];

			//$MAction = D ( 'Addons://Orienteering/TaskAction' );
			$OriWeixin = D ( 'Addons://Orienteering/WeixinAddon' );
			
		    $OriWeixin->addAction ( $task_id,$create_time,$followId,$openid,$token);
		...
		else if ($qr ['addon'] == 'UserCenter') { // 设置用户分组
			$uid = $GLOBALS ['mid'];
			if (empty ( $uid )) {
				$map1 ['openid'] = $dataArr ['FromUserName'];
				$map1 ['token'] = get_token ();
