...
TS.app={
    weibo:{
        ...
        cache:{
            ...
            facePath:SITE_URL+'/addons/theme/stv1/_static/image/expression/default/',
            faceDict:{
                //"shengli": "shengli.gif",
                //"shenma": "shenma.gif",
        ...
        func:{
            readNewMsg:function(){
				if(MID=='0')return;
                var url = SITE_URL+'/index.php?s=/addon/Weiba/Wap/MCount.html';
                var success = function(data){
					var totalCount = data.unread_comment+data.unread_reply_comment+data.unread_event_comment+data.unread_event_reply_comment;
					var topicCount = data.unread_comment+data.unread_reply_comment;
					var eventCount = data.unread_event_comment+data.unread_event_reply_comment;
					if(totalCount>0){
						$('.msg_tips_container .msg').show();
						if(topicCount>0){
							$('#my_topic_msg').text(topicCount).show();
						}else{
							$('#my_topic_msg').hide();
						}
						if(eventCount>0){
							$('#my_event_msg').text(eventCount).show();
						}else{
							$('#my_event_msg').hide();
						}
					}else{
						$('.msg_tips_container .msg').hide();
						$('.navlist .msg').hide();
					}
					if(USER_UN_SETTING=='1'){
						$('.msg_tips_container .msg,#setting_msg').show();
						$('#setting_msg').css('display','inline-block');
					}
					/*xdf_version
...
//                    TS.showMask("密码由字母，数字，符号组成，6-15个字符，区分大小写");
                    ...
                    $.ajax({
                        ...
                        success:function(data,status){
                            if(data.success=='1'){
                                window.location.href=U("w3g/Weiba/index");
                            }else{
...
var loadPageByMore = function(conId,callback){
	if(!conId)conId = '#listData';
	var pageCount = $('#load_page_btn').data('page');
	var pageUrl = $('#load_page_btn').data('url');
	var pageKey = $('#load_page_btn').data('key');
		pageKey=  pageKey?pageKey:'page';
	var currentPage = 1;
	$('#load_page_btn').click(function(){
		currentPage++;
		if(currentPage<=pageCount){
			$('.page_loading').show();
			$.get(pageUrl+'&'+pageKey+'='+currentPage,function(data){
				var tempHtml  = $(data);
				var listHtml = $(conId,tempHtml).html();
				$(conId).append(listHtml);	
				$('.page_loading').hide();
				if(callback){
					callback(data);
				}
