...
var ui = {
	 ...
	 * @return void
	 */
	chooseCity: function(fromCity) {
		$.get(U('w3g/Weiba/getLocalCity'),function(data){
			if(fromCity && fromCity>0){
				if(data!=0){
					var $p = "<p>您当前定位城市为:<span>"+data+"</span></p>";	
					$html = '<div class="choose_city_dialog"><div class="inner">'+$p+'<div class="btnbar"><a href="w3g/Weiba/index">切换城市</a><a href="'+U('w3g/Weiba/switchCity')+'&city='+fromCity+'">不切换</a></div></div></div>';
				}else{
					var $p = "<p>您当前定位城市暂无站点</p>";	
					$html = '<div class="choose_city_dialog"><div class="inner">'+$p+'<div class="btnbar"><a href="'+U('w3g/Weiba/cityList')+'">切换城市</a><a href="'+U('w3g/Weiba/switchCity')+'&city='+fromCity+'">不切换</a></div></div></div>';
				}
			}else{
				if(data!=0){
					var $p = "<p>定位城市为:<span>"+data+"</span></p>";	
					$html = '<div class="choose_city_dialog"><div class="inner">'+$p+'<div class="btnbar"><a href="'+U('w3g/Weiba/cityList')+'">切换城市</a><a href="javascript:$(\'.choose_city_dialog\').remove();">确定</a></div></div></div>';
				}else{
					var $p = "<p>您所在的城市尚未开通论坛</p>";	
					$html = '<div class="choose_city_dialog"><div class="inner">'+$p+'<div class="btnbar"><a href="'+U('w3g/Weiba/cityList')+'">切换城市</a></div></div></div>';
				}
	...
	showShareTips: function() {
		var $html = $('<div class="share_tips_layer"><p>点击右上角&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<br/>分享给小伙伴吧~</p></div>');
		$html.click(function(){
			$html.remove();
		});
		$('body').append($html);
...
$(function() {
	...
	var imgNum=$('.w-h-img').length;
	$('.w-h-img').each(function(index, element) {
        var _img = new Image();
	...
	$('.ninepic_list span').height($('.ninepic_list span').width());
	$('#headMenu').click(function(){
		$(this).find('.head_sub_menu').toggle();
	})
	$('#content').click(function(){
		if($('#headMenu .head_sub_menu').is(':visible')){
			$('#headMenu .head_sub_menu').hide();
		}
	});
});
