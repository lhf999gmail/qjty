...
function favorite_weiba(_ts, id, uid, wid){
    ...
    $.post(SITE_URL+'/index.php?s=/addon/Weiba/Wap/favorite', {'post_id': id, 'post_uid':uid, 'weiba_id':wid}, function(d){
        if(d==1){
             $(_ts).hide();
             $(_ts).siblings().show();
        }else{
...
function unfavorite_weiba(_ts, id, uid, wid){
   ...
   $.post(SITE_URL+'/index.php?s=/addon/Weiba/Wap/unfavorite', {'post_id': id, 'post_uid':uid, 'weiba_id':wid}, function(d){
        if(d==1){
             $(_ts).hide();
             $(_ts).siblings().show();
        }else{
...
function del_weiba_reply(_ts, rid){
    ...
    $.post(SITE_URL+'/index.php?s=addon/Weiba/Wap/delReply', {'widget_appname': 'weiba','reply_id': rid}, function(d){
        if(d==1){
