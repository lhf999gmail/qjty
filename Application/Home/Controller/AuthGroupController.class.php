...
class AuthGroupController extends HomeController {
	...
	public function lists() {
		...
		if ($this->syc_wechat) {
			...
			// 搜索按钮
			$search_url = U ( 'AuthGroup/lists' ,array('mdm'=>$_GET['mdm']));
		...
		foreach ($groupidArr as $gg){
			if($group_uid[$gg]){
				$fmap['uid']=array('in',$group_uid[$gg]);
				$fmap['has_subscribe']=1;
				$hasSub[$gg]=M('public_follow')->where($fmap)->count();
			}
		}
		if ($this->qr_code) {
			foreach ( $list_data ['list_data'] as &$vo ) {
				$vo['count'] = '<a href="'.addons_url('UserCenter://UserCenter/lists',array('group_id'=>$vo['id'])).'"/>'.intval($hasSub[$vo['id']]).'</a>';
				if (! empty ( $vo ['qr_code'] )) {
	...
	function toGroupdetail(){
		...
		redirect(addons_url('UserCenter://UserCenter/lists',array('group_id'=>$group_id)));
	}
