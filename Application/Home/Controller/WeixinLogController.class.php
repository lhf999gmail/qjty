 ...
 * 前台首页控制器
 */
class WeixinLogController extends HomeController {
	...
	public function lists2() {
		...
		/* 查询记录总数 */
		$count = M ()->query ( "SELECT count(1) as num FROM `{$px}weixin_log`" );
...
// 		$list ['list_data'] = M ()->query ( $sql );
		...
		$list ['list_data'] = M('weixin_log')->order(' id DESC')->page($page,$row)->select();
...
;
	...
	function set_meterial() {
		...
		if ($res1) {
			...
			$res = M ( 'weixin_message' )->where ( array (
					'id' => $id 
			) )->save ( $save );
		}
