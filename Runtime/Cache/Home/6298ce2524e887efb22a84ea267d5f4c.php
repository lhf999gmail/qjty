...
.small_pic_list li{ border-top:1px solid #fff; border-bottom:1px solid #ccc; }
.small_pic_list li { display:block; padding:10px 10px 10px 80px; position:relative; min-height:60px; color:#333}
.small_pic_list li :active{ background-color:#CCC}
.small_pic_list li img{ position:absolute; left:10px; top:10px; width:60px; height:60px;}
.small_pic_list li h6{ font-size:18px; font-weight:bold; overflow:hidden; text-overflow:ellipsis; white-space:nowrap}
.small_pic_list li p{ color:#999; font-size:13px; line-height:22px;}
</style>
<body id="weisite">
<div class="container">
<ul class="small_pic_list">
    <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
                ...
                <span class="colorless">[<?php echo (day_format($vo["start_date"])); ?>]</span>
