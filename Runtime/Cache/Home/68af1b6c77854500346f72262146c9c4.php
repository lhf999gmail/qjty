                      ...
                      <?php switch($field["type"]): case "num": ?><input type="text" class="text input-medium" name="<?php echo ($field["name"]); ?>" value="<?php echo ($data[$field['name']]); ?>"><?php break;?>
                        ...
                        <?php case "picture": ?><div class="controls">
                            <div class="upload_row muti_picture_row">
                            	...
                            	<?php if(!empty($data[$field['name']])): $tempArr =$data[$field['name']]; for($i=0;$i<count($tempArr);$i++){ if(!empty($data['id']) && $forms['can_edit']!=1){ echo '<div class="img_item"><input type="hidden" name="'.$field[name].'[]" value="'.$tempArr[$i].'"/><img src="'.get_cover_url($tempArr[$i]).'"/></div>'; }else{ echo '<div class="img_item"><em>X</em><input type="hidden" name="'.$field[name].'[]" value="'.$tempArr[$i].'"/><img src="'.get_cover_url($tempArr[$i]).'"/></div>'; } } endif; ?>
                                
                                <?php if(empty($data[id]) || $forms[can_edit]==1): ?><a class="img_item add_btn" href="javascript:;" onClick="$.WeiPHP.wxChooseImg(this,1,'<?php echo ($field["name"]); ?>[]')"><img src="/qjty/Public/Home/images/add.png"/></a><?php endif; ?>
                            </div>
                          </div><?php break;?>
                        <?php case "file": ?><div class="controls">
                          ...
                          <script type="text/javascript">
								...
								$("#upload_file_<?php echo ($field["name"]); ?>").uploadify({
							        "height"          : 30,
							        "swf"             : "/qjty/Public/static/uploadify/uploadify.swf",
