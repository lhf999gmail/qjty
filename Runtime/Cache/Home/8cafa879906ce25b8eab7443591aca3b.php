...
<div>
<!-- <?php if(is_array($slideshow)): $i = 0; $__LIST__ = $slideshow;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>-->
<?php echo ($vo["info"]); ?>
...
<!--     <?php if(!empty($category)): ?><section>
    	<ul class="icon_lists">
            <?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="single_item m_15" style='opacity: 0.8;'>
                <a class="item p_10 arrow_icon mr_10" href="<?php echo ($vo["url"]); ?>">
                    <span class="icon random_color"><img src="<?php echo ($vo["icon"]); ?>"/></span>
                    <span class="click_item_txt"><?php echo ($vo["title"]); ?></span>
                </a>
